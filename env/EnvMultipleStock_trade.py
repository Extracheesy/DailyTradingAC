...
class StockEnvTrade(gym.Env):
    ...
    def __init__(self, df,day = 0, turbulence_threshold=140 ,initial=True, previous_state=[], model_name='', iteration='', df_trace=''):
        ...
        # Modif CD : Shape = SHAPE_SIZE
        self.observation_space = spaces.Box(low=0, high=np.inf, shape = (SHAPE_SIZE,))
    ...
    def step(self, actions):
        ...
        if self.terminal:
            #if(self.df_trace.trace_mode == "LOG"):
            self.model_name = "A2C"
            self.day_date = str(self.data.datadate[33].values[0])[0:10]
            plt.plot(self.asset_memory,'r')
            plt.savefig('model_saved/results/account_value_trade_{}_{}.png'.format(self.model_name, self.day_date))
            plt.close()
            ...
            #if(self.df_trace.trace_mode == "LOG"):
            df_rewards.to_csv('model_saved/results/account_rewards_trade_{}_{}.csv'.format(self.model_name, self.day_date))
            df_total_value.to_csv('model_saved/results/account_total_value_trade_{}_{}.csv'.format(self.model_name, self.day_date))
